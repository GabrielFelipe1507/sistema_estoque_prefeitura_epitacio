...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///estoque_prefeitura.db'
db = SQLAlchemy(app)
...
with app.app_context():
    ...
    if not Categoria.query.first():
        lista = ["Tubinhos de Tinta", "Cartuchos", "Toner", "Monitores", "CPU's", "Teclados", "Mouses", "Nobrakes", "Estabilizadores", "Notebooks", "Fontes", "Caixinhas de Som", "Coolers", "Baterias de Carro"]
...
def index():
    ...
    # NOVAS FUNCIONALIDADES: Busca e Ordenação
    search_query = request.args.get('search', '')
    sort_order = request.args.get('sort', 'asc') # asc ou desc
    ...
    itens = query.all()
    cores = Cor.query.all()
    return render_template('index.html', itens=itens, cores=cores, categorias=categorias, 
                           cat_ativa=cat_selecionada, search=search_query, sort=sort_order)
...
def add_color():
    ...
    if nova_cor:
        try:
            db.session.add(Cor(nome=nova_cor)); db.session.commit()
...
def add():
    ...
    novo = Item(categoria_nome=cat, descricao=request.form.get('descricao'), patrimonio=request.form.get('patrimonio'),
                quantidade=int(request.form.get('quantidade', 0)), marca=request.form.get('marca'))
    db.session.add(novo)
    db.session.commit()
